...
namespace Dapper
{
    ...
    public static partial class SimpleCRUD
    {
        ...
        public static void SetDialect(Dialect dialect)
        {
            switch (dialect)
            {
                ...
                default:
                    ...
                    _dialect = Dialect.SQLServer;
                    _encapsulation = "{0}";
                    _getIdentitySql = "returning Id into :Id";
                    _getPagedListSql = "SELECT * FROM (SELECT ROW_NUMBER() OVER(ORDER BY {OrderBy}) AS PagedNumber, {SelectColumns} FROM {TableName} {WhereClause}) AS u WHERE PagedNUMBER BETWEEN (({PageNumber}-1) * {RowsPerPage} + 1) AND ({PageNumber} * {RowsPerPage})";
                    break;
            }
        }
        ...
        public static TKey Insert<TKey, TEntity>(this IDbConnection connection, TEntity entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            else if ((keytype == typeof(int) || keytype == typeof(long)) && Convert.ToInt64(idProps.First().GetValue(entityToInsert, null)) == 0)
            {
                //sb.Append(";" + _getIdentitySql);
                sb.Append(" returning ID into :" + idProps[0].Name);
            }
            else
            {
                keyHasPredefinedValue = true;
            ...
            //var r = connection.Query(query, entityToInsert, transaction, true, commandTimeout);

            // var r = connection.Execute(query, entityToInsert);
            var param = new DynamicParameters(entityToInsert);
            param.Add(name: "Id", dbType: DbType.Int32, direction: ParameterDirection.Output);
            var r = connection.Execute(query, param, transaction);

            if (idProps.Count == 0)
            {
                return param.Get<TKey>("PKID");
            }
        ...
        public static int Update<TEntity>(this IDbConnection connection, TEntity entityToUpdate, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            if (!idProps.Any())
            {
                idProps = GetPKIdProperties(entityToUpdate).ToList();
            }
        ...
        //Get all properties that are named Id or have the Key attribute
        //For Inserts and updates we have a whole entity so this method is used
        private static IEnumerable<PropertyInfo> GetPKIdProperties(object entity)
        {
            var type = entity.GetType();
            return GetPKIdProperties(type);
        }

        //Get all properties that are named Id or have the Key attribute
        //For Get(id) and Delete(id) we don't have an entity, just the type so this method is used
        private static IEnumerable<PropertyInfo> GetPKIdProperties(Type type)
        {
            var tp = type.GetProperties().Where(p => p.GetCustomAttributes(true).Any(attr => attr.GetType().Name == typeof(KeyAttribute).Name)).ToList();
            return tp.Any() ? tp : type.GetProperties().Where(p => p.Name.Equals("PKID", StringComparison.OrdinalIgnoreCase));
        }
